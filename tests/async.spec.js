...
describe('async tests', function () {
  ...
  it('empty directory', function (done) {
    fto.createTree(path.join(__dirname, '/fixtures/test1'), function (err, tree) {
      if (err) {
        return done(err);
      }
  ...
  it('directory with files', function (done) {
    fto.createTree(path.join(__dirname, '/fixtures/test2'), function (err, tree) {
      if (err) {
        return done(err);
      }
  ...
  it('directory with files and sub directories', function (done) {
    fto.createTree(path.join(__dirname, '/fixtures/test3'), function (err, tree) {
      if (err) {
        return done(err);
      }
  ...
  it('large volume of directories and files loads ok', function (done) {
    fto.createTree(path.join(__dirname, '../node_modules'), function (err, tree) {
      if (err) {
        return done(err);
      }
  ...
  it('bad directory', function (done) {
    fto.createTree(path.join(__dirname, '/fixtures/testx'), function (err, tree) {
      assert.ok(err, 'error was not returned as expected');
      done();
    });
  });
});
