...
describe('forEach tests', function () {
  ...
  /**
   * Test forEach with early leave
   */
  it('leave iteration early', function () {
    var tree = fto.createTreeSync(path.join(__dirname, '/fixtures/test3'));
    var totalCount = 0;
    var results = {};
    results[path.join(__dirname, '/fixtures/test3')] = 0;
    ...
    assert.equal(results[path.join(__dirname, '/fixtures/test3/file1.txt')], 1, 'file1 file incorrect');
    assert.equal(results[path.join(__dirname, '/fixtures/test3/file2.txt')], 1, 'file2 file incorrect');
    assert.equal(results[path.join(__dirname, '/fixtures/test3/sub/sub2')], 0, 'sub2 directory incorrect');
    assert.equal(results[path.join(__dirname, '/fixtures/test3/sub/sub2/file5.txt')], 0, 'file5 file incorrect');
    assert.equal(results[path.join(__dirname, '/fixtures/test3/sub/file3.txt')], 0, 'file3 file incorrect');
    assert.equal(results[path.join(__dirname, '/fixtures/test3/sub/file4.txt')], 0, 'file4 file incorrect');
  });
});
